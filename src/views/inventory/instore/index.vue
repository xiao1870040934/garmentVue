<template>
  <div>
    ...
    <el-table
      ...
      border
      stripe
      v-show="addForm.detail.length !== 0"
    >
    ...
    <el-dialog title="添加明细" :visible.sync="dialogFormVisible" width="45%">
      ...
      >
        <el-form-item label="货号" prop="goodsId">
          <el-select v-model="detailForm.goodsId" placeholder="请选择">
            <el-option
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addFromRules: {
        ...
        from: [{ required: true, message: "请输入供应商", trigger: "blur" }],
        addTime: [
  ...
  methods: {
    showDialog() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (valid) {
          ...
          let result = await this.$API.goodsManage.getGoodsList();
          if (result.code === 200) {
            this.goodsIds = result.data;
          }
    ...
    addFormConfirm() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (valid) {
          this.addForm.detail.forEach((item1) => {
            const goods = item1.goodsId
            this.goodsIds.forEach((item) => {
              if (item.number == goods) {
                item1.goodsId = item.id;
              }
            });
          });
