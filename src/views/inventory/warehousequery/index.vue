<template>
  <div>
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input placeholder="请选择入库日期" v-model="queryInfo.addTime" clearable @clear="getPageList"></el-input>
        </el-col>
        <el-col :span="8">
            <el-input placeholder="请输入库单号" v-model="queryInfo.number" clearable @clear="getPageList">
      ...
      <el-table :data="list" border stripe style="margin:10px 0px">
        ...
        <el-table-column label="操作" width="width">
            <template slot-scope="scope">
                ...
                <el-button type="warning" icon="el-icon-edit" size="mini" @click="editInstore(scope.row.id)">修改</el-button>               
                <el-button type="danger" icon="el-icon-delete" size="mini" @click="removeInstore(scope.row.id)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <el-pagination
    ...
    <!-- 详情对话框 -->
    <el-dialog title="入库明细" :visible.sync="detailFormVisible" width="45%">
    ...
    <el-dialog
      ...
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="80px">
          ...
          <el-form-item label="供应商" prop="addTime">
      ...
      <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="editPutStoreInfo">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
    ...
    methods: {
      async getPageList() {
        let result= await this.$API.inwharehouse.reqPutStoreList(this.queryInfo)
        if(result.code === 200){
      ...
      async showDetail(id){
        let result= await this.$API.inwharehouse.reqDetailList(id)
        if(result.code === 200){
          this.detailList=result.data
          this.detailFormVisible=true
        }
      },
      async removeInstore(id){
      ...
      async editInstore(id) {
        let result= await this.$API.inwharehouse.getInstoreById(id)
        if(result.code===200){
      ...
      editPutStoreInfo() {
        this.$refs.editFormRef.validate(async valid => {
          if (valid) {
            let result= await this.$API.inwharehouse.editPutStoreInfoById(this.editForm)
            if(result.code===200){
